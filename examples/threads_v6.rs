...
use jira::{get_issues_and_worklogs, get_issues_for_single_project, get_jira_data_from_url, get_worklogs_for, JiraIssue, JiraProject, JiraProjectsPage, midnight_a_month_ago_in, Worklog};
...
async fn get_all_worklogs(http_client: &Client, issue_keys: Vec<String>) -> Vec<Worklog> {
    let result: Vec<Vec<Worklog>> = stream::iter(issue_keys)
        .map(|key| {
            jira::get_worklogs_for(http_client,key, midnight_a_month_ago_in())
        }).buffer_unordered(30).collect().await;

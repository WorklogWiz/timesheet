...
use jira_lib::{get_jira_data_from_url, JiraProject, JiraProjectsPage, midnight_a_month_ago_in, Worklog};
...
*/
    let start = Instant::now();
    let _result = jira_lib::get_all_projects(&http_client, vec![]).await;
    let elapsed = start.elapsed().as_millis();

    println!("all_jira_projects: {}, get_all_projects: {}", elapsed1, elapsed);

    for issue in vec!["TIME-39","RGA-8", "TIME-40"] {
        let worklogs = jira_lib::get_worklogs_for(&http_client, issue.to_string(), midnight_a_month_ago_in()).await;

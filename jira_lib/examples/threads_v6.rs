...
async fn main() {
    ...
    let start = Instant::now();
    let logs = get_all_worklogs(&jira_client.http_client, issue_keys).await;
...
async fn get_all_worklogs(http_client: &Client, issue_keys: Vec<String>) -> Vec<Worklog> {
    let result: Result<Vec<Vec<Worklog>>, StatusCode> = stream::iter(issue_keys)
        .map(|key| {
...
async fn execute_worklogs_futures(http_client: &Client, issues: Vec<String>) -> Vec<Worklog> {
    let result: Result<Vec<Vec<Worklog>>, StatusCode> = worklogs_stream(http_client, issues).buffer_unordered(10).collect().await;
    result.into_iter().flatten().collect()
}
#[allow(dead_code)]
fn worklogs_stream(http_client: &Client, issues: Vec<String>) -> impl Stream<Item=impl Future<Output=Result<Vec<Worklog>, StatusCode>> + '_> + '_{
    stream::iter(issues).map(move |issue| {
        JiraClient::get_worklogs_for(&http_client, issue,midnight_a_month_ago_in() )
    })
}

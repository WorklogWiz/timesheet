...
async fn test_get_worklog_entries_for_current_user(){
    ...
    let r = jira_client.insert_worklog( TEST_ISSUE, Local::now(), 27000, "Rubbish comment").await;
    println!("Received a response: {r:?}");
    assert!(r.is_ok(),"Insertion failed {r:?}");

    println!("Inserted ok");

    let result = jira_client.get_worklogs_for_current_user(TEST_ISSUE,Local::now().checked_sub_days(Days::new(1))).await;
    ...
    let r = r.unwrap();
    let _r = jira_client.delete_worklog(TEST_ISSUE.to_string(), r.id).await;

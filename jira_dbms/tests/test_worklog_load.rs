...
use jira_lib::{ JiraIssuesPage, WorklogsPage};
use jira_lib::Worklog;
...
fn test_find_steinar_time_40() {
    ...
    for w in steinar {
        println!("{} - {} - {} - {}", w.author.displayName ,w.created.date_naive(), w.timeSpentSeconds, w.timeSpent);
        if w.created.date_naive().month() == 2 && w.created.date_naive().year() == 2022 {
            total += w.timeSpentSeconds as i64;
        }

    }
    println!("Total {}", total/3600);
}

#[test]
fn test_parse_date() {
    match DateTime::parse_from_str("2022-02-04T16:22:28.554+0100", "%Y-%m-%dT%H:%M:%S%.f%z") {
        Ok(dt) => {
            println!("Parsed: {dt:?}");
...
async fn test_insert_multiple_worklogs() {
    ...
    println!("Parsing time-40_worklog_results.json took {elapsed}ms");

    jira_dbms::batch_insert_worklogs(&mut dbms, &worklogs_page.worklogs[1..4]).await;
}
...
fn test_chunk_worklogs() {
    ...
    let mut cnt = 0;
    let mut total = 0;
    for chunk in worklogs.chunks(1000) {
        cnt += 1;
        total += 1000;
        println!("Chunk {} contains {}", cnt, chunk.len());
    }
...
fn test_load_with_custom_fields() {
    ...
    assert_eq!(issues[0].fields.asset.as_ref().unwrap().id, "12150");
}
